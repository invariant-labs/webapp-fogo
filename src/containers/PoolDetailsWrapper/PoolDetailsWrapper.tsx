...
import { ALL_FEE_TIERS_DATA, disabledPools, USDC_MAIN, WFOGO_MAIN } from '@store/consts/static'
...
export const PoolDetailsWrapper: React.FC<IProps> = ({
  ...
  useEffect(() => {
    ...
    if (!tokenX) {
      if (tokenY && tokenY.assetAddress.toString() !== WFOGO_MAIN?.address.toString()) {
        tokenX = tokens.find(token => token.address.toString() === WFOGO_MAIN?.address.toString())
      } else {
    ...
    if (!tokenY) {
      if (tokenX && tokenX.assetAddress.toString() !== USDC_MAIN?.address.toString()) {
        ...
        tokenY = tokens.find(token => token.address.toString() === WFOGO_MAIN?.address.toString())
      }
    }
